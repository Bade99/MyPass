...
//Thanks https://code.woboq.org/userspace/glibc/crypt/sha256.c.html
...
# ifdef _MSC_VER
#  define SWAP(n) _byteswap_ulong (n)
#  define SWAP64(n) _byteswap_uint64 (n)
# else
#  define SWAP(n) \
...
__sha256_process_bytes(const void* buffer, size_t len, struct sha256_ctx* ctx)
{
    ...
    if (len >= 64)
    {
        ...
        /* To check alignment gcc has an appropriate operator.  Other
           compilers don't.  */
# if __GNUC__ >= 2
#  define UNALIGNED_P(p) (((uintptr_t) p) % __alignof__ (u32) != 0)
# else

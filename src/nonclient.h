...
LRESULT CALLBACK proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	...
	case WM_MEASUREITEM: //NOTE: this is so stupid, this guy doesnt send hwndItem like WM_DRAWITEM does, so you have no way of knowing which type of menu you get, gotta do it manually
		//TODO(fran): should this and wm_drawitem go in the client?
	{
		return handle_wm_measureitem(hwnd, msg, wparam, lparam, state.menu);
	} break;
	case WM_DRAWITEM:
	{
		return handle_wm_drawitem(hwnd, msg, wparam, lparam, state.menu);
	} break;
	case WM_COMMAND:
	{
